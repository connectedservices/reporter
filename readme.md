# Grafana reporter <img style="float: right;" src="https://travis-ci.org/connectedservices/reporter.svg?branch=master">
...
Get the source files and dependencies:
    ...
    go get github.com/connectedservices/reporter/...

Build and install `grafana-reporter` binary to `$GOPATH/bin`:

    go install -v github.com/connectedservices/reporter/cmd/grafana-reporter
...
Also see [this issue](https://github.com/connectedservices/reporter/issues/50) for an example. 
...
If you also have `Make` and `Docker-compose` installed, you can run a simple local orchestration of Grafana and Grafana-reporter:
     ...
     go get github.com/connectedservices/reporter/ ...
     cd $GOPATH/src/github.com/connectedservices/reporter
     make compose-up
...
The unit tests can be run using the go tool:
    ...
    go test -v github.com/connectedservices/reporter/...

or, the [GoConvey](http://goconvey.co/) webGUI:

    ./bin/goconvey -workDir `pwd`/src/github.com/connectedservices -excludedDirs `pwd`/src/github.com/connectedservices/reporter/tmp/
